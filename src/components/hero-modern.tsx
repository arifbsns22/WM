...
const DeckGlyph = ({ theme = "dark" }) => {
  ...
  const fill =
    theme === "dark" ? "rgba(255,255,255,0.08)" : "rgba(17,17,17,0.08)";
...
function HeroOrbitDeck() {
  ...
  const protocols = [
    ...
    },
    {
      name: "Launch window",
      detail:
        "Go/no-go checks, activate uplink, monitor stability in realtime.",
      status: "Live",
    },
  ];
        ...
        style={{
          background:
            theme === "dark"
              ...
              : "radial-gradient(60% 50% at 50% 10%, rgba(234, 246, 255), transparent 70%)",
          filter: "blur(22px)",
        }}
      />
      ...
      >
        ...
        <div className="grid gap-10 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.2fr)_minmax(0,0.9fr)] xl:items-stretch">
          ...
          >
            ...
            <p className={`text-sm leading-relaxed ${palette.subtle}`}>
              Designed for monochrome libraries where clarity outruns color.
              Craft a hero that introduces the system, shows live state, and
              frames imagery without losing contrast discipline.
            </p>
            <div className="grid gap-3">
              {[
                "Interface parity guaranteed",
                "Cross-theme neutral hues",
                "Motion tuned for calm focus",
              ].map((item) => (
                <div
                  key={item}
                  className="relative overflow-hidden rounded-2xl border px-4 py-3 text-xs uppercase tracking-[0.3em] transition duration-500 hover:-translate-y-0.5 hover:shadow-[0_14px_40px_rgba(0,0,0,0.18)] dark:hover:shadow-[0_14px_40px_rgba(0,0,0,0.45)]"
                >
                  <span>{item}</span>
                  <span
          ...
          <figure
            className="order-1 overflow-hidden rounded-[32px] border transition xl:order-2"
            ...
            <ul className="space-y-4">
              {protocols.map((protocol, index) => (
                ...
                >
                  ...
                  <p
                    className={`mt-3 text-sm leading-relaxed ${palette.subtle}`}
                  >
                    {protocol.detail}
                  </p>
                </li>
